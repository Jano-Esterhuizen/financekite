    ...
    IValidator<UpdateClientRequest> updateValidator)
{
        ...
        Guid id, Guid businessId, Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        var outstandingBalance = client.Invoices
            ...
            .Sum(i => i.Amount);
        return new ClientHealthResponse(client.Id, client.Name, totalInvoiced, outstandingBalance);
    }

    public async Task<ClientResponse> CreateAsync(
        Guid businessId, Guid userId, CreateClientRequest request, CancellationToken cancellationToken = default)
    {
        await VerifyBusinessOwnershipAsync(businessId, userId, cancellationToken);
    ...
    public async Task<ClientResponse> UpdateAsync(
        Guid id, Guid businessId, Guid userId, UpdateClientRequest request, CancellationToken cancellationToken = default)
    {
        await VerifyBusinessOwnershipAsync(businessId, userId, cancellationToken);
